...
- 建造按“绑定锚点 → 主背包 → 快捷栏 → 副手”取材；材料不足的位置保留黄色虚影，右键聚焦任一虚影继续补齐。准星射线进入黄色方格或蓝色选区/部署框的边线与内部体积都会取得唯一焦点并加粗，部署框优先，其余对象按入射距离选择最近者；左键只取消当前焦点，且部署取消优先于源选区和缺料任务。大型预览按 16³ 分区缓存实体 QUADS 线框、投影模型和六面蓝紫裂罅波纹；预览距离至少 160 格并随客户端渲染距离提高到最多 512 格，128 格外自动保留完整框架与波纹 LOD，避免近中距离缺块及大型蓝图瞬时烘焙卡顿。
...
- 按住 `Alt` 会立即显示裂罅风格的暗紫径向轮盘；选中扇区使用青紫辉光、放大徽标和中央状态提示，松开后按原状态机关闭或确认调整。
