...
- 建造按“绑定锚点 → 主背包 → 快捷栏 → 副手”取材；材料不足的位置保留黄色虚影，右键任一虚影继续补齐。准星命中黄色虚框或蓝色任务边框时边框会加粗，只有此时左键才会取消对应任务。大型预览按 16³ 分区缓存 GPU 线框，并对方块虚影采用近距帧预算，避免稳定画面逐格重建。
- 拆除只有在副手工具类型和等级均合格时才保留正常掉落；每个成功拆除的方块消耗一次工具耐久，不生成经验。
- 共鸣供材锚潜行贴附在容器面上使用。它优先访问目标面的 Forge 物品 Capability，并兼容 `WorldlyContainer`、普通 `Container` 和可配置的无侧面 Capability 回退；锚点默认私有，可设置公开或白名单。
- 蓝图保存在客户端全局目录 `<游戏目录>/mydimension/blueprints/`，文件扩展名为 `.mindbp`，可跨存档和服务器导入、导出。服务器会重新验证尺寸、权限、材料、目标方块及完整方块实体 NBT。
